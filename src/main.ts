...
export class Jupyter extends intellij.Disposable {
    ...
    private activate() {
        ...
        LanguageProviders.getInstance().on('onLanguageProviderRegistered', (language: string) => {
            this.disposables.push(intellij.languages.registerCodeLensProvider(language, this.codeLensProvider));
            this.disposables.push(intellij.languages.registerDocumentSymbolProvider(language, symbolProvider));
        });
    ...
    private executeCodeInKernel(kernel: Kernel.IKernel, code: string): Rx.Observable<ParsedIOMessage> {
        if (this.jupyterVersionWorksWithJSServices) {
            return Rx.Observable.create<ParsedIOMessage>(observer => {
    ...
    async executeSelection(): Promise<any> {
        const activeEditor = intellij.window.activeTextEditor;
    ...
    private registerCommands() {
        this.disposables.push(intellij.commands.registerCommand(Commands.Jupyter.ExecuteRangeInKernel, (document: intellij.TextDocument, range: intellij.Range) => {
        ...
        this.disposables.push(intellij.commands.registerCommand(Commands.Jupyter.SelectExistingNotebook, () => {
