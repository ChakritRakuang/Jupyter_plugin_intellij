...
export class JupyterClientAdapter extends EventEmitter implements IJupyterClientAdapter {
    ...
    public start(envVariables?: { [key: string]: string }): Promise<any> {
        ...
        const pyFile = path.join(__dirname, '..', '..', '..', '..', 'python', 'PythonTools', 'ipythonServer.py');
        const newEnv = {};
    ...
    public getAllKernelSpecs(): Promise<Kernel.ISpecModels> {

        return this.start()
            .then(() => this.ipythonAdapter.listKernelSpecs())
            .then(specs => {
                let kernelspecs: { [key: string]: Kernel.ISpecModel } = {};
                Object.keys(specs).forEach(key => {
                    kernelspecs[key] = specs[key].spec;
                    // When we get the spec from iPython code (that we've written)
                    // the name property is empty
                    (kernelspecs[key] as any).name = kernelspecs[key].name || key
                });
                return { default: '', kernelspecs } as Kernel.ISpecModels;
            });
    }
    private lastStartedKernelUUID: string;
    public startKernel(kernelSpec: Kernel.ISpecModel): Promise<[string, any, string]> {
        return this.start()
            .then(() => this.getAllKernelSpecs())
            .then((specks: Kernel.ISpecModels) => {
                // ok given the specks, find the name of the kernelspec
                const kernelSpecName = Object.keys(specks.kernelspecs).find(kernelSpecName => {
                    const spec = specks.kernelspecs[kernelSpecName];
                    return spec.display_name === kernelSpec.display_name;
                });
                return this.ipythonAdapter.startKernel(kernelSpec.name).then(info => {
    ...
    public runCode(code: string): Rx.Observable<ParsedIOMessage> {
        // const subject = new Rx.Subject<ParsedIOMessage>();
        const subject: Rx.Subject<ParsedIOMessage> = new (Rx as any).ReplaySubject();
        this.start().then(() => {
            const runnerObservable = this.ipythonAdapter.runCode(code);
            runnerObservable.subscribe(data => {
                if (data.data === 'ok' && data.type === 'text' && data.stream === 'status'){
                    return;
                }
                subject.onNext(data);
            }, reason => {
